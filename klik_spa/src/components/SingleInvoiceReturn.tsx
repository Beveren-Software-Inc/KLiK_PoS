...
export default function SingleInvoiceReturn({
    ...
    try {
      ...
      if (invoiceDetails.success && invoiceDetails.data) {
        ...
        items.push({
          ...
          amount,
          returned_qty: returnedQty,
          available_qty: Math.round((qty - returnedQty) * 1000) / 1000,  // Round to 3 decimal places
          return_qty: Math.round((qty - returnedQty) * 1000) / 1000  // Round to 3 decimal places
        });
      }
  ...
  const handleReturnQtyChange = (itemCode: string, newQty: number) => {
    setReturnItems(prev => prev.map(item => {
      if (item.item_code === itemCode) {
        // Ensure return qty doesn't exceed available qty and round to 3 decimal places
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center justify-center space-x-2">
                              <button
                                onClick={() => handleReturnQtyChange(
                                  item.item_code,
                                  Math.round(((item.return_qty || 0) - 1) * 1000) / 1000  // Round to 3 decimal places
                                )}
                                disabled={!item.return_qty || item.return_qty <= 0}
                                className="p-1 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-100 dark:hover:bg-gray-600 rounded"
                              >
                              ...
                              <input
                                ...
                                onChange={(e) => handleReturnQtyChange(
                                  item.item_code,
                                  Math.round((parseFloat(e.target.value) || 0) * 1000) / 1000  // Round to 3 decimal places
                                )}
                                className="w-24 px-2 py-1 text-center border border-gray-300 dark:border-gray-600 rounded focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm"
                                disabled={item.available_qty === 0}
                                step="0.001"
                              />
                              <button

    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(STORES.PRODUCTS)) {
          const productsStore = db.createObjectStore(STORES.PRODUCTS, { keyPath: "itemCode" })
        ...
        if (!db.objectStoreNames.contains(STORES.PENDING_INVOICES)) {
          const invoicesStore = db.createObjectStore(STORES.PENDING_INVOICES, { keyPath: "tempId" })
          invoicesStore.createIndex("timestamp", "timestamp", { unique: false })
        }

        // Customers store
        if (!db.objectStoreNames.contains(STORES.CUSTOMERS)) {
          const customersStore = db.createObjectStore(STORES.CUSTOMERS, { keyPath: "name" })
